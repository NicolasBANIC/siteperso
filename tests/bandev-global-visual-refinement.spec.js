...
test.describe('BANDEV Global Visual Refinement Validation', () => {
  ...
  test('should have no parasitic separators between sections', async ({ page }) => {
    ...
    const separators = page.locator('.section-separator, .band, .band--green, .band--grey, .separator, .gradient-strip, .divider, .section-divider');
  ...
  test('should have consistent card styling throughout homepage', async ({ page }) => {
    ...
    const cards = page.locator('.card, .service-card, .pricing-card, .testimonial-card, article[class*="card"]');
  ...
  test('should maintain gradient consistency across different pages', async ({ page }) => {
    ...
    for (const testPage of pages) {
      ...
      expect(bodyBackground, `${testPage.name} page should have gradient background`).toContain('linear-gradient');
      ...
      const sectionsWithBackground = await page.locator('section').evaluateAll(sections => {
        return sections.filter(section => {
          ...
          return bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent' && !section.classList.contains('hero');
        }).length;
      });
      
      expect(sectionsWithBackground, `${testPage.name} page should not have sections with background overrides`).toBe(0);
    }
  });
  ...
  test('should have seamless hero to content section transition', async ({ page }) => {
    ...
    if (await heroSection.isVisible()) {
      ...
      const nextSection = page.locator('.hero + section, .hero-section + section, section[class*="hero"] + section').first();
  ...
  test('should maintain responsive gradient consistency', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      expect(bodyBackground, `${viewport.name} viewport should maintain gradient`).toContain('linear-gradient');
      ...
      expect(mainBackground, `${viewport.name} main should be transparent`).toMatch(/transparent|rgba\(0,\s*0,\s*0,\s*0\)/);
    }
  });
});