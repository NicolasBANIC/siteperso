...
test.describe('BANDEV Harmonization & À Propos Removal Validation', () => {
  ...
  test('should not display "À propos" links in header navigation', async ({ page }) => {
    ...
    const hasAPropos = headerLinkTexts.some(text => 
      text.toLowerCase().includes('à propos') || 
      text.toLowerCase().includes('a propos')
    );
    
    expect(hasAPropos, 'Header should not contain "À propos" link').toBeFalsy();
    
    // Verify expected navigation items are present
    const expectedHeaderLinks = ['Accueil', 'Services', 'Processus', 'Portfolio', 'Tarifs', 'Témoignages', 'Contact'];
  ...
  test('should not display "À propos" links in footer navigation', async ({ page }) => {
    ...
    const hasAPropos = footerLinkTexts.some(text => 
      text.toLowerCase().includes('à propos') || 
      text.toLowerCase().includes('a propos')
    );
  ...
  test('should display hero sections without dark overlay filters', async ({ page }) => {
    ...
    for (let i = 0; i < heroCount; i++) {
      ...
      const overlayElements = hero.locator('[class*="overlay"], [class*="backdrop"], .hero-overlay, .section-overlay');
      ...
      if (beforePseudoElement.backgroundColor !== 'rgba(0, 0, 0, 0)' && 
  ...
  test('should display emerald-turquoise gradient on keyword text in headings', async ({ page }) => {
    // Find gradient text elements (keywords)
    const gradientTexts = page.locator('.heading-gradient, .gradient-text, h1 .gradient, h2 .gradient, [class*="gradient"]');
    ...
    if (await gradientTexts.count() > 0) {
      ...
      const hasEmeraldColors = gradientColors.includes('16, 163, 74') || // #16A34A
  ...
  test('should display buttons with rounded corners and emerald-turquoise gradient', async ({ page }) => {
    // Find CTA buttons
    ...
    for (let i = 0; i < Math.min(buttonCount, 3); i++) {
      ...
      const hasEmeraldBg = background.backgroundColor.includes('16, 163, 74') || 
                          background.backgroundColor.includes('15, 143, 114') ||
                          background.backgroundColor.includes('0, 200, 180');
      
      expect(hasGradient || hasEmeraldBg, `Button ${i + 1} should have gradient or emerald background`).toBeTruthy();
    }
  });
  ...
  test('should have white background on homepage', async ({ page }) => {
    ...
    const isWhiteOrLight = bgColor.includes('255, 255, 255') || 
  ...
  test('should display cards with harmonized gradient backgrounds', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(cardCount, 3); i++) {
      ...
      const hasGradient = background.backgroundImage.includes('linear-gradient');
      const hasWhiteBg = background.backgroundColor.includes('255, 255, 255') ||
                        background.backgroundColor === 'white';
      
      expect(hasGradient || hasWhiteBg, `Card ${i + 1} should have gradient or white background`).toBeTruthy();
  ...
  test('should maintain harmonization across different viewport sizes', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      if (await gradientElements.count() > 0) {
        ...
        expect(backgroundImage, `${viewport.name}: Should maintain gradient text`).toContain('linear-gradient');
      }
      ...
      if (await buttons.isVisible()) {
        ...
        const radiusValue = parseInt(borderRadius);
        expect(radiusValue, `${viewport.name}: Buttons should maintain rounded corners`).toBeGreaterThan(10);
      }
    }
  });

  test('should display text readability enhancement in hero without global dark overlay', async ({ page }) => {
    ...
    if (await heroSection.isVisible()) {
      ...
      if (await heroText.count() > 0) {
        ...
        const hasReadabilityEnhancement = styles.textShadow !== 'none' ||
                                         styles.backgroundColor !== 'rgba(0, 0, 0, 0)' ||
                                         styles.backdropFilter !== 'none' ||
                                         styles.webkitBackdropFilter !== 'none';
        
        // Text should be readable without requiring global dark overlay
        expect(hasReadabilityEnhancement, 'Hero text should have readability enhancement').toBeTruthy();
      }
    }
  });
